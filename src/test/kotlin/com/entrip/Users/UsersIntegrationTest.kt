    ...
    init {
        ...
        given("존재하는 user_id와 nickname으로") {
            ...
            `when`("nickname의 존재 여부를 확인하면") {
                then("true가 리턴된다") {
                    val result = mockMvc.get("/api/v2/users/{nickname}/nickname/exist", nickname) {
                        contentType = MediaType.APPLICATION_JSON
                    }.andExpect {
                        status { isOk() }
                        content { json(objectMapper.writeValueAsString(successExpectedResponseForNickname)) }
                    }.andReturn()
            ...
            `when`("user_id의 존재 여부를 확인하면") {
                then("true가 리턴된다") {
                    mockMvc.get("/api/v2/users/{user_id}/user_id/exist", user_id) {
                        contentType = MediaType.APPLICATION_JSON
                    }.andExpect {
                        status { isOk() }
                        content { json(objectMapper.writeValueAsString(successExpectedResponseForUserId)) }
                    }
                }
            }
        }
        ...
        given("저장되지 않은 nickname과 user_id가 주어졌을 때") {
            ...
            `when`("nickname의 존재 여부를 확인하면") {
                then("HttpStatus.Accepted()를 리턴한다") {
                    mockMvc.get("/api/v2/users/{nickname}/nickname/exist", "invalid" + nickname) {
                        contentType = MediaType.APPLICATION_JSON
                    }.andExpect {
                        status { isAccepted() }
                        content { json(objectMapper.writeValueAsString(expectedErrorResponse)) }
                    }
                }
            }
            `when`("user_id의 존재 여부를 확인하면") {
                then("HttpStatus.Accepted()를 리턴한다") {
                    mockMvc.get("/api/v2/users/{user_id}/user_id/exist", "invalid" + user_id) {
                        contentType = MediaType.APPLICATION_JSON
                    }.andExpect {
                        status { isAccepted() }
                        content { json(objectMapper.writeValueAsString(expectedErrorResponse)) }
        ...
        given("올바른 accessToken을 Http Header에 실어서 (1)") {
            ...
            val expectedResponse = RestAPIMessages(
                httpStatus = 200,
                message = "Update user $user_id's token : $tokenValue",
                data = usersReturnDto
            )
            `when`("addToken을 실행하면") {
                then("token이 더해진 UsersReturnDto가 리턴된다") {
                    mockMvc.put("/api/v1/users/token/{user_id}/{token}", user_id, "tokenValue") {
                        contentType = MediaType.APPLICATION_JSON
        ...
        given("올바른 accessToken을 Http Header에 실어서 (2)") {
            ...
            `when`("존재하는 아이디로 findById를 실행하면") {
                then("user_id로 검색해서 나온 UsersReturnDto가 리턴된다") {
                    mockMvc.get("/api/v1/users/{user_id}", user_id) {
                        contentType = MediaType.APPLICATION_JSON
                        header("AccessToken", accessToken)
                    }.andExpect {
                        status { isOk() }
            ...
            `when`("존재하지 않는 아이디로 findById를 실행하면") {
                then("HttpStatus.Internal_Server_Error를 리턴한다") {
                    mockMvc.get("/api/v1/users/{user_id}", "invalid" + user_id) {
                        contentType = MediaType.APPLICATION_JSON
                        header("AccessToken", accessToken)
                    }.andExpect {
        ...
        given("올바른 accessToken을 Http Header에 실어서 (3)") {
            ...
            val successExpectedResponseWithUserId = RestAPIMessages(
                httpStatus = 200,
                message = "Get user with nicknameOrUserId : $user_id",
                data = usersReturnDto
            )
            val successExpectedResponseWithNickname = RestAPIMessages(
                httpStatus = 200,
                message = "Get user with nicknameOrUserId : $nickname",
                data = usersReturnDto
            )
            val failExpectedResponse = RestAPIMessages(
                httpStatus = 202,
            ...
            `when`("존재하는 아이디로 findUserWithNicknameOrUserId를 실행하면") {
                then("해당 아이디 사용자의 UsersReturnDto를 반환한다") {
                    mockMvc.get("/api/v1/users/findUserWithNicknameOrUserId/{nicknameOrUserId}", user_id) {
                        ...
                        status { isOk() }
                        content { json(objectMapper.writeValueAsString(successExpectedResponseWithUserId)) }
                    }
                }
            }
            ...
            `when`("존재하는 않는 아이디로 findUserWithNicknameOrUserId를 실행하면") {
                then("HttpStatus.Accepted를 리턴한다") {
                    mockMvc.get("/api/v1/users/findUserWithNicknameOrUserId/{nicknameOrUserId}", "invalid" + user_id) {
                        contentType = MediaType.APPLICATION_JSON
                        header("AccessToken", accessToken)
                    }.andExpect {
                        status { isAccepted() }
            ...
            `when`("존재하는 않는 닉네임으로 findUserWithNicknameOrUserId를 실행하면") {
                then("HttpStatus.Accepted를 리턴한다") {
                    mockMvc.get("/api/v1/users/findUserWithNicknameOrUserId/{nicknameOrUserId}", "invalid" + nickname) {
                        contentType = MediaType.APPLICATION_JSON
                        header("AccessToken", accessToken)
                    }.andExpect {
                        status { isAccepted() }
        ...
        given("planner id가 1,2인 두 개의 플래너가 저장되어 있을 때") {
            ...
            val expectedResponse1 = RestAPIMessages(
                httpStatus = 200,
                message = "Add planner, id : 1 with user, id : $user_id",
                data = user_id
            )

            val expectedResponse2 = RestAPIMessages(
                httpStatus = 200,
                message = "Add planner, id : 2 with user, id : $user_id",
                data = user_id
            )
            ...
            val expectedResponseForFindAll = RestAPIMessages(
                httpStatus = 200,
                message = "Load all planners with user, id : $user_id",
                data = expectedList
            )

            `when`("addPlanner로 플래너 1번을 등록하면") {
                then("플래너가 사용자에게 등록되고 planner_id=1가 리턴된다") {
                    mockMvc.put("/api/v1/users/{planner_id}/{user_id}", 1, user_id) {
                        contentType = MediaType.APPLICATION_JSON
                        header("AccessToken", accessToken)
                    }.andExpect {
                        status { isOk() }
                        content { json(objectMapper.writeValueAsString(expectedResponse1)) }
                    }
                }
            }
            ...
            `when`("이후에 올바른 userId로 findAllPlannersWithUserId를 실행하면") {
                then("planner 1번, 2번이 list로 리턴된다") {
                    mockMvc.get("/api/v1/users/{user_id}/all", user_id) {
                        contentType = MediaType.APPLICATION_JSON
                        header("AccessToken", accessToken)
                    }.andExpect {
                        status { isOk() }
                        content { json(objectMapper.writeValueAsString(expectedResponseForFindAll)) }
                    }
                }
            }
        }
